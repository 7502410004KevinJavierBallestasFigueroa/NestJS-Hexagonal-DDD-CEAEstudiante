...
export class DomainExceptionFilter implements ExceptionFilter {
    ...
    const map = new Map<Function, { status: number; message: string }>([
      ...
      [EstudianteNotFoundException, { status: HttpStatus.NOT_FOUND, message: exception.message }],
      [EmailEstudianteDuplicadoException, { status: HttpStatus.CONFLICT, message: exception.message }],
    ]);
